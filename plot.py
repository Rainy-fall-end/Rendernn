...
def plot1():
    ...
    with open("datas/sph_6.json") as f:
        datas = json.load(f)
        for data in datas:
            X.append(float(data["point_sph"][0]))
            Y.append(float(data["point_sph"][1]))
    ...
    ax1.scatter(X, Y, Z1, c=normalized_Z1, cmap=cmap, marker='o')
    ...
    ax2.scatter(X, Y, Z2, c=normalized_Z2, cmap=cmap, marker='^')
    ...
    ax3.scatter(X, Y, Z3, c=normalized_Z3, cmap=cmap, marker='x')
